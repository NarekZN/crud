...
<a href="{{route('create')}}" class="btn btn-primary btn-md " role="button"><i class=" p-1 fas fa-plus"></i>Create Product</a>
    ...
    <tbody>
            ...
            @foreach($products as $product)
                ...
                <td > <a href="{{route('show' , $product)}}"> {{ $product->product }} </a> </td>
                <td> <a href="{{route('show' , $product)}}"> {{ $product->price }} </a> </td>
                <td>
                    <form method="POST" action="{{route('destroy', $product)}}">
                        <a class="btn btn-warning col-4" href="{{route('edit', $product)}}" role="button"><i class="fas fa-pencil-alt p-1"></i>Edit</a>
                        @csrf
                        @method("DELETE")
